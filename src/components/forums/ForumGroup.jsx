    ...
    <div className="forum-group">
      <div className="forum-group-header">
        ...
        <div className="fgh-stats">
          {(group.forumCount ?? group.categories?.length ?? 0)} forums
          {group.postCount != null && (
            <>
              {' '}&middot;{' '}
              {Number(group.postCount).toLocaleString()} posts
            </>
          )}
        </div>
      </div>
          ...
          <Link key={cat.id} to={`/forums/category/${cat.id}`} className={`forum-cat ${cat.isNew ? 'has-new' : ''}`}>
            ...
            <div className="cat-info-cell">
              ...
              {cat.latestUser && (
                <div className="cat-latest">
                  <div className={`cat-latest-av ${cat.latestUser.color || ''}`}>
                    {cat.latestUser.initials || '?'}
                  </div>
            ...
            <div className="cat-stats-cell">
              <div className="cat-stat">
                <div className="cat-stat-num">{Number(cat.posts || 0).toLocaleString()}</div>
                <div className="cat-stat-label">Posts</div>
              </div>
              <div className="cat-stat">
                <div className="cat-stat-num">{Number(cat.threads || 0).toLocaleString()}</div>
                <div className="cat-stat-label">Threads</div>
              </div>
              <div className="cat-stat">
                <div className="cat-stat-num">{Number(cat.views || 0).toLocaleString()}</div>
